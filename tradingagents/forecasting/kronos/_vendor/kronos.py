...
from .module import *  # noqa: F403  (vendored Kronos: star-import of model modules)
...
def auto_regressive_inference(tokenizer, model, x, x_stamp, y_stamp, max_context, pred_len, clip=5, T=1.0, top_k=0, top_p=0.99, sample_count=5, verbose=False, return_samples=False):
    with torch.no_grad():
        ...
        preds = z.cpu().numpy()
        if return_samples:
            return preds  # (batch, sample_count, seq, feat) — per-sample paths, un-averaged
...
class KronosPredictor:
    ...
    def predict_samples(self, df, x_timestamp, y_timestamp, pred_len, T=1.0, top_k=0, top_p=0.9, sample_count=30, verbose=False):
        """Like ``predict`` but returns the per-sample paths instead of their mean.

        Returns a numpy array of shape ``(sample_count, pred_len, n_features)`` with the
        feature order ``[open, high, low, close, volume, amount]`` (close = index 3),
        de-normalized to price space. Lets callers build a forecast distribution
        (the public ``predict`` averages the samples and discards the spread).
        """
        if not isinstance(df, pd.DataFrame):
            raise ValueError("Input must be a pandas DataFrame.")
        if not all(col in df.columns for col in self.price_cols):
        ...
        preds = self.generate(x, x_stamp, y_stamp, pred_len, T, top_k, top_p, sample_count, verbose, return_samples=True)
